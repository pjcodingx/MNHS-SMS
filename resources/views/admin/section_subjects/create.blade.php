@extends('layouts.admin')

@section('content')
<div class="dashboard-content">

    <div style="margin-bottom: 20px; display: flex; justify-content: space-between; align-items: center;">
        <h2 style="color: #1f2937;">Add Section Subject</h2>
        <a href="{{ route('admin.section_subjects.index') }}"
           style="background: linear-gradient(45deg, #3b82f6, #1d4ed8);
                  color: white; padding: 10px 20px; border-radius: 10px; text-decoration: none;
                  font-weight: 500; transition: all 0.3s ease;">
            <i class="fas fa-arrow-left"></i> Back
        </a>
    </div>

    @if ($errors->any())
        <div style="padding: 12px; background:#fee2e2; border:1px solid #f87171; color:#b91c1c; border-radius:10px; margin-bottom:20px;">
            <ul style="margin:0; padding-left:20px;">
                @foreach ($errors->all() as $error)
                    <li>{{ $error }}</li>
                @endforeach
            </ul>
        </div>
    @endif

    <div style="background: white; padding: 20px; border-radius: 15px; box-shadow: 0 5px 20px rgba(0,0,0,0.1); max-width: 700px;">
        <form action="{{ route('admin.section_subjects.store') }}" method="POST">
            @csrf

            {{-- Grade Level + Section --}}
            <div style="margin-bottom: 15px;">
                <label for="section_id" style="display:block; font-weight:600; margin-bottom:5px;">Grade Level + Section</label>
                <select name="section_id" id="section_id" style="width:100%; padding:8px; border-radius:8px; border:1px solid #3b82f6;">
                    <option value="">Select Grade Level + Section</option>
                    @foreach($sections as $section)
                        <option value="{{ $section->id }}" data-grade="{{ $section->grade_level_id }}">
                            {{ $section->gradeLevel->name ?? 'No Grade Assigned' }} - {{ $section->name }}
                        </option>
                    @endforeach
                </select>
            </div>

            {{-- Subject --}}
            <div style="margin-bottom: 15px;">
                <label for="subject_id" style="display:block; font-weight:600; margin-bottom:5px;">Subject (Only for selected Grade Level)</label>
                <select name="subject_id" id="subject_id" style="width:100%; padding:8px; border-radius:8px; border:1px solid #3b82f6;">
                    <option value="">Select Subject</option>
                    @foreach($subjects as $subject)
                        <option value="{{ $subject->id }}" data-grade="{{ $subject->grade_level_id }}">
                            {{ $subject->name }}
                        </option>
                    @endforeach
                </select>
            </div>

            {{-- Schedule --}}
            <div style="margin-bottom: 15px;">
                <label for="schedule_id" style="display:block; font-weight:600; margin-bottom:5px;">Schedule (Filtered by Grade Level + Section + Subject)</label>
                <select name="schedule_id" id="schedule_id" style="width:100%; padding:8px; border-radius:8px; border:1px solid #3b82f6;">
                    <option value="">Select Schedule</option>
                    @foreach($schedules as $schedule)
                        <option value="{{ $schedule->id }}" data-grade="{{ $schedule->grade_level_id }}" data-subject="{{ $schedule->subject_id }}">
                            {{ implode('-', $schedule->days ?? []) }} {{ $schedule->time_start }}–{{ $schedule->time_end }}
                        </option>
                    @endforeach
                </select>
            </div>

            {{-- Adviser --}}
            <div style="margin-bottom: 15px;">
                <label for="adviser_id" style="display:block; font-weight:600; margin-bottom:5px;">Adviser</label>
                <select name="adviser_id" id="adviser_id" style="width:100%; padding:8px; border-radius:8px; border:1px solid #3b82f6;">
                    <option value="">Select Adviser</option>
                    @foreach($advisers as $adviser)
                        <option value="{{ $adviser->id }}">{{ $adviser->name }}</option>
                    @endforeach
                </select>
            </div>

            <button type="submit"
                    style="background: linear-gradient(45deg, #3b82f6, #1d4ed8); color:white; padding:10px 20px; border-radius:10px; border:none; cursor:pointer; font-weight:500;">
                Add Assignment
            </button>
        </form>
    </div>
</div>

<script>
document.addEventListener('DOMContentLoaded', function() {
    const sectionSelect = document.getElementById('section_id');
    const subjectSelect = document.getElementById('subject_id');
    const scheduleSelect = document.getElementById('schedule_id');

    const allSubjects = Array.from(subjectSelect.options);
    const allSchedules = Array.from(scheduleSelect.options);

    sectionSelect.addEventListener('change', function() {
        const selectedGrade = this.selectedOptions[0]?.dataset.grade;

        // Filter subjects
        subjectSelect.innerHTML = '<option value="">Select Subject</option>';
        allSubjects.forEach(option => {
            if(option.value === "" || option.dataset.grade === selectedGrade) {
                subjectSelect.appendChild(option.cloneNode(true));
            }
        });

        // Reset schedule
        scheduleSelect.innerHTML = '<option value="">Select Schedule</option>';
    });

    subjectSelect.addEventListener('change', function() {
        const selectedGrade = sectionSelect.selectedOptions[0]?.dataset.grade;
        const selectedSubject = this.value;

        // Filter schedules
        scheduleSelect.innerHTML = '<option value="">Select Schedule</option>';
        allSchedules.forEach(option => {
            if(option.value === "") return;
            if(option.dataset.grade === selectedGrade && option.dataset.subject === selectedSubject) {
                scheduleSelect.appendChild(option.cloneNode(true));
            }
        });
    });
});

</script>
@endsection
