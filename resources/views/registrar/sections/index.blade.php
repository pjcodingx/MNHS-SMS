    ...
    <div style="background:white; border-radius:15px; box-shadow:0 5px 20px rgba(0,0,0,0.1); padding:30px;">
        ...
        <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:30px; padding-bottom:20px; border-bottom:2px solid #e5e7eb;">
            <div>
                ...
                <p style="color:#6b7280; font-size:14px;">View all sections and their students</p>
            </div>
        </div>

        <!-- Filters Section -->
        <form method="GET" action="{{ route('registrar.sections.index') }}" style="background:#f9fafb; padding:20px; border-radius:12px; margin-bottom:25px;">
            <div style="display:grid; grid-template-columns:1fr 1fr 1fr auto; gap:15px; align-items:end;">
                ...
                <div style="display:flex; gap:10px;">
                    ...
                    @if(request()->hasAny(['grade_level_id', 'strand_id', 'search']))
                ...
                @foreach($sections as $section)
                    <a href="{{ route('registrar.sections.show', $section->id) }}"
                        ...
                        <div style="width:50px; height:50px; background:linear-gradient(135deg, #3b82f6, #1d4ed8); border-radius:12px; display:flex; align-items:center; justify-content:center; margin-bottom:15px;">
                            <i class="fas fa-door-open" style="color:white; font-size:24px;"></i>
                        </div>

                        <!-- Section Name -->
                        <h3 style="color:#1f2937; font-size:20px; font-weight:700; margin-bottom:10px;">
                            {{ $section->name }}
                        </h3>

                        <!-- Grade Level & Strand -->
                        <div style="margin-bottom:15px;">
                            <div style="display:inline-block; background:#dbeafe; color:#1e40af; padding:5px 12px; border-radius:20px; font-size:12px; font-weight:600; margin-right:8px; margin-bottom:5px;">
                                <i class="fas fa-layer-group" style="margin-right:5px;"></i>
                                {{ $section->gradeLevel->name ?? 'N/A' }}
                            </div>
                            @if($section->strand)
                                <div style="display:inline-block; background:#fef3c7; color:#92400e; padding:5px 12px; border-radius:20px; font-size:12px; font-weight:600;">
                                    <i class="fas fa-stream" style="margin-right:5px;"></i>
                                    {{ $section->strand->name }}
                                </div>
                            @endif
            ...
            <div style="margin-top:30px; display:flex; justify-content:center; align-items:center; gap:5px;">
                ...
                @foreach ($sections->getUrlRange(1, $sections->lastPage()) as $page => $url)
                    ...
                    style="padding:10px 16px; border-radius:8px; border:2px solid #3b82f6;
                            {{ $sections->currentPage() == $page ? 'background:#3b82f6; color:white;' : 'background:white; color:#3b82f6;' }}
                            text-decoration:none; font-weight:600; transition:all 0.3s;"
                    @if($sections->currentPage() != $page)
        ...
        @else
            <div style="text-align:center; padding:60px 20px; background:#f9fafb; border-radius:12px;">
                <i class="fas fa-door-open" style="font-size:64px; color:#d1d5db; margin-bottom:20px;"></i>
                <h3 style="color:#6b7280; font-size:18px; margin-bottom:10px;">No Sections Found</h3>
